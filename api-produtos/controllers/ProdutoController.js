...
class ProdutoController {
    ...
    static async criarProduto(req, res) {
        try {
            const { nome, preco, descricao, id_categoria, fornecedor, tipo, especificacoes, ativo } = req.body;
            ...
            if (preco === undefined || preco === null || preco === "") {
                return res.status(400).json({ sucesso: false, erro: 'Preço obrigatório' });
            }

            const precoNumber = Number(preco);

            if (isNaN(precoNumber) || precoNumber <= 0) {
                return res.status(400).json({ sucesso: false, erro: 'Preço deve ser um número maior que zero' });
            }
    ...
    static async atualizarProduto(req, res) {
        try {
            ...
            if (preco !== undefined) {
                ...
                if (isNaN(precoNumber) || precoNumber <= 0) {
                    return res.status(400).json({ sucesso: false, erro: 'Preço inválido' });
                }

                dadosAtualizacao.preco = precoNumber;
            }
